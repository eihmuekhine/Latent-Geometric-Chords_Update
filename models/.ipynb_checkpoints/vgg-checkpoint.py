...
class VGG(nn.Module):
    # Updated defaults for CIFAR-10
    def __init__(self, configs, num_classes=10, img_size=32, enable_bn=False):
        super(VGG, self).__init__()
        self.encoder = VGGEncoder(configs=configs, enable_bn=enable_bn)
        
        # 5 MaxPool layers reduce spatial dimensions by 2^5 = 32
...
if __name__ == "__main__":
    # Updated input dimensions for CIFAR-10 (Batch Size 5, 3 Channels, 32x32 Image)
    input_tensor = torch.randn((5, 3, 32, 32))
    configs = get_configs('vgg16')
    
    # Test AutoEncoder
    ae_model = VGGAutoEncoder(configs)
    ae_output = ae_model(input_tensor)
    print("AutoEncoder Output Shape:", ae_output.shape) # Expected: [5, 3, 32, 32]
    
    # Test Classifier
    classifier_model = VGG(configs)
    classifier_output = classifier_model(input_tensor)
    print("Classifier Output Shape:", classifier_output.shape) # Expected: [5, 10]